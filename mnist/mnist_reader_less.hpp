...
namespace mnist {
	 ...
	 */
	template <typename Pixel = float, typename Label = uint>
		...
		if (buffer) {
			...
			for (size_t i = 0; i < count; ++i) {
				...
				for (size_t j = 0; j < rows * columns; ++j) {
					...
					images[i][j] = static_cast<Pixel>(pixel) / 255;
				}
			}
	...
	std::vector<std::vector<Pixel>> read_training_images() {
		return read_mnist_image_file<Pixel>("../TreeOfLife/src/mnist/train-images-idx3-ubyte");
	}
	...
	std::vector<std::vector<Pixel>> read_test_images() {
		return read_mnist_image_file<Pixel>("../TreeOfLife/src/mnist/t10k-images-idx3-ubyte");
	}
	...
	std::vector<Label> read_training_labels() {
		return read_mnist_label_file<Label>("../TreeOfLife/src/mnist/train-labels-idx1-ubyte");
	}
	...
	std::vector<Label> read_test_labels() {
		return read_mnist_label_file<Label>("../TreeOfLife/src/mnist/t10k-labels-idx1-ubyte");
	}
